...
export class DatasetSectionKind {
	...
	// todo only 2 tests fails
	// public static processCourseFiles(content: string): Promise<InsightResult[]> {
	// 	return this.extractCourseFiles(content)
	// 		.then((jsonCourseFiles) => {
	// 			const sectionsArray: InsightResult[] = [];
	// 			let hasValidSection = false;
	//
	// 			const promises = jsonCourseFiles.map((jsonCourseFile) => {
	...
	// }
	//
	// private static extractCourseFiles(content: string): Promise<string[]> {
	// 	let zip = new JSZip();
	...
	// 		})
	// 		.catch((error) => {
	// 			throw new InsightError("Error extracting course files: " + error.message);
	// 		});
	// }
